...
beta20 = 2.0
#################################
ltime  = 3*700*len(ld['se']) # int(3600.*kyori/lknot)
for epi in range(100):
  gpserr = cn.powerlaw_psd_gaussian(beta15, ltime)
  gpe = ghptb * (gpserr[                  : 700*len(ld['se'])] - np.average(gpserr[                  : 700*len(ld['se'])]))
  gpn = ghptb * (gpserr[ 700*len(ld['se']):1400*len(ld['se'])] - np.average(gpserr[ 700*len(ld['se']):1400*len(ld['se'])]))
  gpu = gvptb * (gpserr[1400*len(ld['se']):2100*len(ld['se'])] - np.average(gpserr[1400*len(ld['se']):2100*len(ld['se'])]))
#################################
