...
solverc4.solve()
#####################
#一番重要な補正パート（計算値 - 場の平均値による計算値 + 理論値）
#（平均計算値 - 理論値）が離散化による誤差の推定値
#GNSS-Aシミュレータでは海表面分布だけを考慮するが、ある程度似たものになるため、この補正で10^-5sec精度に達する
#####################
ttb = solverb1.traveltime.values
ttc = solverc1.traveltime.values
ttm = 1.e3*np.sqrt((solverc1.velocity.nodes[:,:,:,0]-solverc1.velocity.nodes[slon1,sdep1,slat1,0])**2+(solverc1.velocity.nodes[:,:,:,1]-solverc1.velocity.nodes[slon1,sdep1,slat1,1])**2+(solverc1.velocity.nodes[:,:,:,2]-solverc1.velocity.nodes[slon1,sdep1,slat1,2])**2)/tsv
ttd1 = ttc - ttb + ttm
#####################
ttb = solverb2.traveltime.values
ttc = solverc2.traveltime.values
ttm = 1.e3*np.sqrt((solverc2.velocity.nodes[:,:,:,0]-solverc2.velocity.nodes[slon2,sdep2,slat2,0])**2+(solverc2.velocity.nodes[:,:,:,1]-solverc2.velocity.nodes[slon2,sdep2,slat2,1])**2+(solverc2.velocity.nodes[:,:,:,2]-solverc2.velocity.nodes[slon2,sdep2,slat2,2])**2)/tsv
ttd2 = ttc - ttb + ttm
#####################
ttb = solverb3.traveltime.values
ttc = solverc3.traveltime.values
ttm = 1.e3*np.sqrt((solverc3.velocity.nodes[:,:,:,0]-solverc3.velocity.nodes[slon3,sdep3,slat3,0])**2+(solverc3.velocity.nodes[:,:,:,1]-solverc3.velocity.nodes[slon3,sdep3,slat3,1])**2+(solverc3.velocity.nodes[:,:,:,2]-solverc3.velocity.nodes[slon3,sdep3,slat3,2])**2)/tsv
ttd3 = ttc - ttb + ttm
#####################
ttb = solverb4.traveltime.values
ttc = solverc4.traveltime.values
ttm = 1.e3*np.sqrt((solverc4.velocity.nodes[:,:,:,0]-solverc4.velocity.nodes[slon4,sdep4,slat4,0])**2+(solverc4.velocity.nodes[:,:,:,1]-solverc4.velocity.nodes[slon4,sdep4,slat4,1])**2+(solverc4.velocity.nodes[:,:,:,2]-solverc4.velocity.nodes[slon4,sdep4,slat4,2])**2)/tsv
ttd4 = ttc - ttb + ttm
#####################
...
#print(ttd[:,0,slon])
ax11.plot([int(slon*dlon), int(slat*dlat)], [0, int(2*slat*dlat)], 'w-', lw=2)
ax11.plot([0, int(2*slat*dlat)], [int(slon*dlon), int(slat*dlat)], 'w-', lw=2)
ax11.plot([int(slon*dlon)*(1-1/math.sqrt(2)), int(slat*dlat)*(1+1/math.sqrt(2))], [int(slon*dlon)*(1-1/math.sqrt(2)), int(slat*dlat)*(1+1/math.sqrt(2))], 'w-', lw=2)
ax11.plot([int(slon*dlon)*(1-1/math.sqrt(2)), int(slat*dlat)*(1+1/math.sqrt(2))], [int(slon*dlon)*(1+1/math.sqrt(2)), int(slat*dlat)*(1-1/math.sqrt(2))], 'w-', lw=2)
ax11.plot([int(slon*dlon), int(2*slat*dlat), int(slon*dlon), 0, int(slon*dlon)], [0, int(slat*dlat), int(2*slon*dlon), int(slon*dlon), 0], 'w-', lw=2)
ax11.plot([int(slon*dlon), 1.5*slat*dlat, int(slon*dlon), 0.5*slat*dlat, int(slon*dlon)], [0.5*slon*dlon, int(slat*dlat), 1.5*slon*dlon, int(slat*dlat), 0.5*slon*dlon], 'w-', lw=2)
ax11.plot(int(slon*dlon), int(slat*dlat), marker='*',markersize=4)
ax11.set_xlim([0,int(2*slon*dlon)])
ax11.set_ylim([0,int(2*slat*dlat)])
...
#################################
#データの抽出　色々とややこしいことをやっている　測線上を等間隔に、海底局ごとの違いを取り出している
#################################
beta10 = 1.0
beta15 = 1.5
beta20 = 2.0
...
#################################
    ...
    stime = np.linspace(n*100.*math.ceil(36.*kyori/lknot), n*100.*math.ceil(36.*kyori/lknot)+3600.*kyori/lknot, len(sewr))
...
#データの書き出し
#################################
  df = pd.DataFrame({'SET': da[0][0], 'LN': da[0][1],'MT': da[0][2],'TT': da[0][3],'ResiTT': np.nan,'TakeOff': np.nan,'ST': da[0][4],'ant_e0': da[0][5]*1.e3,'ant_n0': da[0][6]*1.e3,'ant_u0': da[0][7]*1.e3,'head0': 0.0,'pitch0': 0.0,'roll0': 0.0,'dSV0': 0.0,'RT': da[0][8],'ant_e1': da[0][9]*1.e3,'ant_n1': da[0][10]*1.e3,'ant_u1': da[0][11]*1.e3,'head1': 0.0,'pitch1': 0.0,'roll1': 0.0,'dSV1': 0.0,'flag': da[0][12]})
