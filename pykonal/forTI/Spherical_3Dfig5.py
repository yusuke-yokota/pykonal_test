...
#m_s_dp=[   15,   24,   30]
#m_s_km=[ 0.87,-0.25,  0.0] # cm/s/km !!!! REVERSE setting !!!!
...
for j in range(0,m_s_dp[0]):
  ...
  for i in range(nth):
    for p in range(nph):
      a      = [sv.speed[j]*0.001+m_s_km[0]*0.001*0.001*float(p-50)]
...
for j in range(m_s_dp[0],m_s_dp[1]):
  ...
  for i in range(nth):
    for p in range(nph):
      a      = [sv.speed[j]*0.001+m_s_km[1]*0.001*0.001*float(p-50)]
      svd[i] = np.append(svd[i], a)
  a1[j] = [svd[0]]
  for i in range(1,nth):
    a1[j] = np.append(a1[j], [svd[i]], axis=0)
for j in range(m_s_dp[1],nrh):
  ...
  for i in range(nth):
    for p in range(nph):
      a      = [sv.speed[j]*0.001+m_s_km[2]*0.001*0.001*float(p-50)]
      svd[i] = np.append(svd[i], a)
  a1[j] = [svd[0]]
  for i in range(1,nth):
    a1[j] = np.append(a1[j], [svd[i]], axis=0)
...
ax11.contour(
    ...
    tt1.values[0,:,:]-tt0.values[0,:,:],
    colors="k",
    levels=np.arange(-0.1, 0.1, 0.0003),
    linewidths=0.2,
    linestyles="--"
)
cbar = fig.colorbar(qmesh, cax=cax01, orientation="horizontal", ticks=[-0.01,-0.005,0.005,0.01])
...
#shutil.copyfile("figureS.png", "/mnt/owncloud_webdav/webdav/figureS.png")
