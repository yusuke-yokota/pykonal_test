...
##
#sv = pd.read_csv('svpF.csv')
#sv = pd.read_csv('svpE.csv')
sv = pd.read_csv('out.csv')
##
...
#dlat,ddep,dlon=0.010,0.010,0.010
...
for i in range(ndep):
  svdo[i] = np.array([(tov[i])*0.0010000] * nlon)
  svd[i] = np.array([(sv.speed[i])*0.0010000] * nlon)
svpo = [svdo[0]]
svp = [svd[0]]
for i in range(1,ndep):
  svpo = np.append(svpo, [svdo[i]], axis=0)
  svp = np.append(svp, [svd[i]], axis=0)
o0 = np.array([svpo] * nlat)
a0 = np.array([svp] * nlat)
solverb.velocity.values = o0
...
qmesh = ax11.pcolormesh(
    solverc.traveltime.nodes[:,0,:,0],
    solverc.traveltime.nodes[:,0,:,2],
    tt0[:,0,:],
#    ttc[:,0,:],
    cmap=plt.get_cmap("coolwarm"),
    norm=Normalize(vmin=-0.00002,vmax=0.00002)
)
ax11.contour(
    solverc.traveltime.nodes[:,0,:,0],
    solverc.traveltime.nodes[:,0,:,2],
...
#    ttc[:,0,:],
    colors="k",
    levels=np.arange(tt0.min(), tt0.max(), 0.000001),
    linewidths=0.2,
    linestyles="--"
)
...
ax11.set_xlim([0,2])
ax11.set_ylim([0,2])
cbar = fig.colorbar(qmesh, cax=cax01, orientation="horizontal", ticks=[-0.00001,0.0,0.00001])
cbar.set_label("")
cbar.set_clim(-0.00002,0.00002)
#cbar.ax.xaxis.tick_top()
#cbar.ax.xaxis.set_label_position("top")
...
xn=solverc.traveltime.nodes[:,0,slon,0].reshape(-1,)
yn=solverc.traveltime.nodes[:,0,slon,1].reshape(-1,)
zn=solverc.traveltime.nodes[:,0,slon,2].reshape(-1,)
